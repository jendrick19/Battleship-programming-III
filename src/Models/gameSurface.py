...
class GameSurface:
    ...
    def handle_events(self, events):
        if self.state == "setup":
            # Track which ship is currently being dragged
            ...
            for event in events:
                # Handle global mouse up event first to ensure dragging state is reset
                if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                    # If any ship is being dragged, end its drag
                    if currently_dragging:
                        currently_dragging.end_drag(self.gridSz, self.ships)
                        # Force update all ships to ensure none are in dragging state
                        for ship in self.ships:
                            ship.dragging = False
                        continue  # Skip processing this event for other ships
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                        for ship in self.ships:
                            if ship.handle_event(event, self.offset_x, self.offset_y, self.cellSz, self.gridSz, self.ships):
                                ...
                                break
                    # For other event types, process for all ships
                    else:
                        for ship in self.ships:
                ...
                if not self.action_taken:
                    ...
                    if event.type == pygame.KEYDOWN and self.active:
                        if event.key == pygame.K_RETURN:
                            ...
                            if len(self.input_text) < 3 and (event.unicode.isalnum() or event.unicode.isspace()):
    ...
    def handle_attack_input(self, input_text):
        ...
        if not input_text[0].isalpha():
            self.error_message = "First character must be a letter!"
            self.active = False
            return self.error_message

        # Extract the numeric part (can be more than one digit)
        letter_part = input_text[0].upper()
        ...
        if 0 <= row < self.gridSz and 0 <= col < self.gridSz:
            self.handle_attack(None, row, col)
            self.action_taken = True
            self.active = False
        else:
